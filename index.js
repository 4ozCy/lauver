...
function buildNowPlaying(queue, track) {
  ...
  // Add buttons inside the container using addActionRowComponents
...
async function handleModal(interaction) {
  ...
  if (type === "volume") {
    ...
    try { const msg = await interaction.channel.messages.fetch(parts[2]); await msg.edit(buildNowPlaying(queue, queue.current)); } catch {}
    return;
  }
