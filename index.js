...
function buildNowPlaying(queue, track) {
  const loopIcon = ["🔄 Off","🔂 Track","🔁 Queue","♾️ Auto"][queue.repeatMode] ?? "🔄 Off";
  ...
  c.addTextDisplayComponents(tx(
    ...
    "  ·  Queue: " + (queue.tracks.size + 1) + " track(s)  ·  Loop: " + loopIcon
  ));
  ...
  const row2 = new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId("music:voldown").setLabel("🔊 Volume").setStyle(ButtonStyle.Secondary),
    new ButtonBuilder().setCustomId("music:clearqueue").setLabel("🗑 Clear Queue").setStyle(ButtonStyle.Secondary)
  );
  return { components: [c, row, row2], flags: F, allowedMentions: { parse: [] } };
}

player.events.on("playerStart", (queue, track) => {
  queue.metadata.channel.send(buildNowPlaying(queue, track));
});

player.events.on("emptyQueue", (queue) => {
  const c = mc(0x5865f2);
  c.addTextDisplayComponents(tx("## 🎵 Queue Ended\nNo more tracks. Add more with " + PREFIX + "play!"));
...
const OWNER_ID       = "1107744228773220473";
...
reg("General", "botperms", ["bp","selfperms"], "Show the bot's permissions in this channel", ".botperms", async (msg) => {
  const perms   = msg.channel.permissionsFor(msg.guild.members.me);
  if (!perms) return sendErr(msg.channel, "Could not fetch bot permissions.");
  const allowed = Object.entries(PermissionFlagsBits)
    .filter(([, bit]) => perms.has(bit))
    .map(([name]) => name.replace(/([A-Z])/g, " $1").trim());
  const missing = Object.entries(PermissionFlagsBits)
    .filter(([, bit]) => !perms.has(bit))
    .map(([name]) => name.replace(/([A-Z])/g, " $1").trim());
  const c = mc();
  c.addTextDisplayComponents(tx("## 🔐 Bot Permissions — #" + msg.channel.name));
  c.addSeparatorComponents(sp());
  c.addTextDisplayComponents(tx(
...
reg("General", "whitelist", ["w","wa","wr","wl"], "Manage the bot whitelist (owner only)", ".whitelist add/remove/list @user|ID", async (msg, args, invokedAs) => {
  if (msg.author.id !== OWNER_ID) return;
  const aliasMap = { wa: "add", wr: "remove", wl: "list" };
  if (aliasMap[invokedAs]) args.unshift(aliasMap[invokedAs]);
  ...
  if (sub === "add") {
    ...
    if (target.id === OWNER_ID) return sendErr(msg.channel, "You cannot remove yourself from the whitelist.");
    whitelist.delete(target.id);
    sendOk(msg.channel, "## ✅ Removed\n**" + (target.tag ?? target.id) + "** can no longer use the bot.");
  } else if (sub === "list") {
    const c = mc();
    c.addTextDisplayComponents(tx("## 📋 Whitelist\n" + [...whitelist].map((id, i) => (i+1) + ".  <@" + id + "> (" + id + ")").join("\n")));
    send(msg.channel, c);
  } else {
    sendErr(msg.channel, "Usage: " + PREFIX + "whitelist add / remove / list");
  }
});
...
async function handleButton(interaction) {
  ...
  if (id.startsWith("openmodal:")) {
    ...
    if (isNaN(vol) || vol < 1 || vol > 100) {
      const c = mc(0xed4245); c.addTextDisplayComponents(tx("❌  Volume must be between 1 and 100."));
    ...
    try {
      ...
      await msg.edit(buildNowPlaying(queue, queue.currentTrack));
    } catch {}
    return;
  }
  ...
  if (id.startsWith("music:")) {
    ...
    if (action === "stop") {
      queue.delete();
      await interaction.deferUpdate();
      const c = mc(0xed4245); c.addTextDisplayComponents(tx("## ⏹️ Stopped\nQueue cleared and left the voice channel."));
      return interaction.message.edit({ components: [c], flags: F, allowedMentions: { parse: [] } });
    }
    if (action === "playpause") {
      queue.node.isPaused() ? queue.node.resume() : queue.node.pause();
      await interaction.deferUpdate();
      const msg = buildNowPlaying(queue, queue.currentTrack);
      return interaction.message.edit(msg);
    }
    if (action === "shuffle") {
      if (queue.tracks.size < 2) {
        const c = mc(0xed4245); c.addTextDisplayComponents(tx("❌  Not enough tracks to shuffle."));
        return interaction.reply({ components: [c], flags: F | MessageFlags.Ephemeral, allowedMentions: { parse: [] } });
      }
    ...
    if (action === "voldown" || action === "volup") {
      return interaction.showModal(
        buildModal("modal:volume:" + interaction.message.id, "Set Volume",
          textInput("volume", "Volume (1–100)", TextInputStyle.Short, {
            placeholder: "Enter a number between 1 and 100",
            value: String(queue.node.volume),
            max: 3, min: 1
          })
        )
      );
    ...
    if (action === "prev") {
      try {
        ...
        const c = mc(0xed4245); c.addTextDisplayComponents(tx("❌  No previous track in history."));
        return interaction.reply({ components: [c], flags: F | MessageFlags.Ephemeral, allowedMentions: { parse: [] } });
      }
      return;
    }
...
reg("General", "serverroles", ["sr","listroles","roles"], "List all roles in the server", ".serverroles", async (msg) => {
  if (!msg.guild) return;
  const roles = [...msg.guild.roles.cache.filter(r => r.id !== msg.guild.id).values()].sort((a,b) => b.position - a.position);
  sendPaged(msg.channel, msg.author.id, roles, 15,
    r => "**" + r.name + "** — " + r.members.size + " members  ·  " + r.hexColor,
    "🎭 Roles (" + roles.length + ")"
  );
});

reg("General", "serverchannels", ["sc","listchannels","channels"], "List all channels in the server", ".serverchannels", async (msg) => {
  if (!msg.guild) return;
  const channels = [...msg.guild.channels.cache.filter(c => c.type === 0 || c.type === 2).values()];
  sendPaged(msg.channel, msg.author.id, channels, 15,
    c => (c.type === 0 ? "#" : "🔊 ") + c.name,
    "📺 Channels (" + channels.length + ")"
  );
});

reg("General", "bots", ["listbots","botlist"], "List all bots in the server", ".bots", async (msg) => {
  if (!msg.guild) return;
  await msg.guild.members.fetch();
  const bots = [...msg.guild.members.cache.filter(m => m.user.bot).values()];
  sendPaged(msg.channel, msg.author.id, bots, 15, m => m.user.tag, "🤖 Bots (" + bots.length + ")");
});

reg("General", "newmembers", ["new","newest","latest"], "Show the newest members", ".newmembers", async (msg) => {
  if (!msg.guild) return;
  await msg.guild.members.fetch();
  const sorted = [...msg.guild.members.cache.filter(m => !m.user.bot).values()]
...
reg("General", "nicknames", ["nicks","listnicks"], "List members with custom nicknames", ".nicknames", async (msg) => {
  if (!msg.guild) return;
  await msg.guild.members.fetch();
  const nicked = [...msg.guild.members.cache.filter(m => m.nickname).values()];
  if (!nicked.length) return sendErr(msg.channel, "No members have custom nicknames.");
  sendPaged(msg.channel, msg.author.id, nicked, 15,
    m => m.user.tag + " → **" + m.nickname + "**",
    "✏️ Nicknames (" + nicked.length + ")"
  );
});
...
reg("General", "id", ["getid","snowflake"], "Get the ID of a user, role, or channel", ".id [@user|@role|#channel]", async (msg) => {
  ...
  if (!target) {
    const c = mc();
    c.addTextDisplayComponents(tx("## 🔢 Your ID\n" + msg.author.id));
    return send(msg.channel, c);
  }
  const c = mc();
  c.addTextDisplayComponents(tx("## 🔢 ID\n**" + (target.tag ?? target.name ?? target.name) + "**\n" + target.id));
  send(msg.channel, c);
});

reg("General", "uptime", ["ut","up"], "Show how long the bot has been running", ".uptime", async (msg) => {
  const c = mc();
  c.addTextDisplayComponents(tx("## ⏱️ Uptime\n" + fmtDur(process.uptime() * 1000)));
  send(msg.channel, c);
});

reg("General", "lookup", ["fetchuser","getuser"], "Fetch a user by ID (even if not in server)", ".lookup <userID>", async (msg, args) => {
  if (!args[0]) return sendErr(msg.channel, "Usage: " + PREFIX + "lookup <userID>");
  let user;
  try { user = await client.users.fetch(args[0], { force: true }); }
  catch { return sendErr(msg.channel, "User not found for ID: " + args[0]); }
  const c = mc();
...
reg("Tools", "tinyurl", ["shorten","shorturl","tiny"], "Shorten a URL", ".tinyurl <url>", async (msg, args) => {
  if (!args[0]) return sendErr(msg.channel, "Usage: " + PREFIX + "tinyurl <url>");
  let data;
  try { data = await fetchJSON("https://tinyurl.com/api-create.php?url=" + encodeURIComponent(args[0])); }
  catch { return sendErr(msg.channel, "Could not shorten URL."); }
  const c = mc();
  c.addTextDisplayComponents(tx("## 🔗 Shortened URL\n**Original:** " + args[0] + "\n**Short:** " + data));
  send(msg.channel, c);
});


reg("Tools", "percentage", ["pct","percent","perc"], "Calculate a percentage", ".percentage <value> <total>", async (msg, args) => {
  ...
  const c = mc();
  c.addTextDisplayComponents(tx("## % Percentage\n**" + val + "** out of **" + total + "** = **" + pct + "%**"));
  send(msg.channel, c);
});

reg("Tools", "currency", ["convert","fx","exchange"], "Convert currency (USD base)", ".currency <amount> <from> <to>", async (msg, args) => {
  ...
  let data;
  try { data = await fetchJSON("https://open.er-api.com/v6/latest/" + from); }
  catch { return sendErr(msg.channel, "Could not fetch exchange rates."); }
  if (data.result !== "success") return sendErr(msg.channel, "Invalid currency: " + from);
  const rate = data.rates[to];
  if (!rate) return sendErr(msg.channel, "Unknown target currency: " + to);
  const c = mc(0x57f287);
...
reg("Fun", "trivia2", ["t2","fasttrivia","qt"], "Get a true/false trivia question", ".trivia2", async (msg) => {
  let data;
  try { data = await fetchJSON("https://opentdb.com/api.php?amount=1&type=boolean"); }
  catch { return sendErr(msg.channel, "Could not fetch trivia."); }
  const q = data.results?.[0];
  if (!q) return sendErr(msg.channel, "No trivia received.");
  const c = mc(0x5865f2);
  c.addTextDisplayComponents(tx("## 🧠 True or False?"));
  c.addSeparatorComponents(sp());
  c.addTextDisplayComponents(tx(
    "**Category:** " + htmlDecode(q.category) + "\n**Difficulty:** " + q.difficulty + "\n\n" +
  ...
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("trivia:true").setLabel("True").setStyle(ButtonStyle.Success),
    new ButtonBuilder().setCustomId("trivia:false").setLabel("False").setStyle(ButtonStyle.Danger)
  );
  msg.channel.send({ components: [c, row], flags: F, allowedMentions: { parse: [] } });
});


reg("Fun", "slot", ["slots","slotmachine","jackpot"], "Play the slot machine", ".slot", async (msg) => {
  const symbols = ["🍒","🍋","🍊","🍇","⭐","💎","🎰","7️⃣"];
  const reels   = Array.from({length:3}, () => symbols[Math.floor(Math.random()*symbols.length)]);
  const win     = reels.every(r => r === reels[0]);
  const jackpot = win && reels[0] === "7️⃣";
...
reg("Fun", "number", ["guessnumber","numgame"], "Guess a number between 1 and 100", ".number", async (msg) => {
  const secret = Math.floor(Math.random() * 100) + 1;
  const c = mc(0x5865f2);
  c.addTextDisplayComponents(tx("## 🎲 Number Game\nI'm thinking of a number between **1** and **100**.\nType your guess in chat within 30 seconds!"));
  await send(msg.channel, c);
  const filter = m => m.author.id === msg.author.id && !isNaN(parseInt(m.content));
  try {
    const collected = await msg.channel.awaitMessages({ filter, max: 5, time: 30000, errors: ["time"] });
    for (const [, m] of collected) {
      const guess = parseInt(m.content);
      if (guess === secret) {
        return sendOk(msg.channel, "## 🎉 Correct!\n**" + msg.author.username + "** guessed **" + secret + "**!");
      }
      const hint = mc(0xfee75c);
      hint.addTextDisplayComponents(tx(guess < secret ? "📈 Too low!" : "📉 Too high!"));
      send(msg.channel, hint);
    }
    sendErr(msg.channel, "Out of guesses! The number was **" + secret + "**.");
  } catch { sendErr(msg.channel, "Time's up! The number was **" + secret + "**."); }
});


reg("Fun", "poll2", ["multipoll","mpoll"], "Create a multi-choice poll", ".poll2 <question> | <opt1> | <opt2> ...", async (msg, args) => {
...
reg("Fun", "never", ["nhie","neverhavei"], "Never have I ever question", ".never", async (msg) => {
  const questions = [
    ...
    "Never have I ever taken a photo and immediately deleted it.",
...
reg("Fun", "thisorthat", ["tot","either","thisorthat"], "This or that question", ".thisorthat", async (msg) => {
  const questions = [
    ["Coffee", "Tea"], ["Beach", "Mountains"], ["Morning", "Night"],
    ["Cats", "Dogs"], ["Books", "Movies"], ["Summer", "Winter"],
    ["Pizza", "Burgers"], ["iOS", "Android"], ["Netflix", "YouTube"],
    ["City", "Countryside"], ["Fast food", "Home cooking"], ["Spotify", "Apple Music"],
  ];
  const [a, b] = questions[Math.floor(Math.random()*questions.length)];
  const c = mc(0xeb459e);
  c.addTextDisplayComponents(tx("## 🤔 This or That?\n**" + a + "**  or  **" + b + "**?"));
  const row = new ActionRowBuilder().addComponents(
    new ButtonBuilder().setCustomId("tot:a:" + a).setLabel(a).setStyle(ButtonStyle.Primary),
    new ButtonBuilder().setCustomId("tot:b:" + b).setLabel(b).setStyle(ButtonStyle.Secondary)
  );
  msg.channel.send({ components: [c, row], flags: F, allowedMentions: { parse: [] } });
});


reg("Moderation", "lockall", ["lockserver","serverlock"], "Lock all text channels", ".lockall", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageChannels, PermissionFlagsBits.Administrator))
    return sendErr(msg.channel, "Missing Administrator permission.");
  sendConfirm(msg,
    "Lock **all text channels** in this server? (@everyone will not be able to send messages.)",
    async () => {
      const channels = msg.guild.channels.cache.filter(c => c.type === 0);
...
reg("Moderation", "unlockall", ["unlockserver","serverunlock"], "Unlock all text channels", ".unlockall", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageChannels, PermissionFlagsBits.Administrator))
    return sendErr(msg.channel, "Missing Administrator permission.");
  sendConfirm(msg,
    "Unlock **all text channels** in this server?",
    async () => {
      const channels = msg.guild.channels.cache.filter(c => c.type === 0);
      for (const [, ch] of channels)
...
reg("Moderation", "mentionrole", ["pingr","pmrole","mr2"], "Toggle if a role is mentionable", ".mentionrole @role", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageRoles)) return sendErr(msg.channel, "Missing ManageRoles permission.");
  const role = msg.mentions.roles.first();
  if (!role) return sendErr(msg.channel, "Usage: " + PREFIX + "mentionrole @role");
  await role.setMentionable(!role.mentionable);
  sendOk(msg.channel, "## ✅ Role Updated\n**" + role.name + "** is now " + (role.mentionable ? "not " : "") + "mentionable.");
});

reg("Moderation", "hoistrole", ["hoistrole","hr2"], "Toggle if a role is hoisted (shown separately)", ".hoistrole @role", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageRoles)) return sendErr(msg.channel, "Missing ManageRoles permission.");
  const role = msg.mentions.roles.first();
  if (!role) return sendErr(msg.channel, "Usage: " + PREFIX + "hoistrole @role");
  await role.setHoist(!role.hoist);
  sendOk(msg.channel, "## ✅ Role Updated\n**" + role.name + "** is now " + (!role.hoist ? "not " : "") + "hoisted.");
});

reg("Moderation", "createrole", ["newrole","mkrole","cr3"], "Create a new role", ".createrole <name> [#hex]", async (msg, args) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageRoles)) return sendErr(msg.channel, "Missing ManageRoles permission.");
  if (!args.length) return sendErr(msg.channel, "Usage: " + PREFIX + "createrole <name> [#hex]");
  const hex  = args.find(a => /^#?[0-9a-fA-F]{6}$/.test(a))?.replace("#","");
  const name = args.filter(a => !/^#?[0-9a-fA-F]{6}$/.test(a)).join(" ");
  if (!name) return sendErr(msg.channel, "Please provide a role name.");
  const role = await msg.guild.roles.create({ name, color: hex ? parseInt(hex,16) : 0, reason: "Created by " + msg.author.tag });
  const c = mc(role.color || 0x5865f2);
  c.addTextDisplayComponents(tx("## ✅ Role Created\n**" + role.name + "**  ·  ID: " + role.id + (hex ? "  ·  #" + hex.toUpperCase() : "")));
  send(msg.channel, c);
});

reg("Moderation", "deleterole", ["rmrole","remrole","dr2"], "Delete a role", ".deleterole @role", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageRoles)) return sendErr(msg.channel, "Missing ManageRoles permission.");
  const role = msg.mentions.roles.first();
  if (!role) return sendErr(msg.channel, "Usage: " + PREFIX + "deleterole @role");
  sendConfirm(msg,
...
reg("Moderation", "createchannel", ["mkchannel","addchannel","cc2"], "Create a text channel", ".createchannel <name>", async (msg, args) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageChannels)) return sendErr(msg.channel, "Missing ManageChannels permission.");
  const name = args.join("-").replace(/\s+/g,"-").toLowerCase();
  if (!name) return sendErr(msg.channel, "Usage: " + PREFIX + "createchannel <name>");
  const ch = await msg.guild.channels.create({ name, type: 0, reason: "Created by " + msg.author.tag });
  sendOk(msg.channel, "## ✅ Channel Created\n" + ch + "  ·  ID: " + ch.id);
});

reg("Moderation", "deletechannel", ["rmchannel","delchannel","dc2"], "Delete a channel", ".deletechannel [#channel]", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageChannels)) return sendErr(msg.channel, "Missing ManageChannels permission.");
  const ch = msg.mentions.channels.first() ?? msg.channel;
  sendConfirm(msg,
    "Delete **#" + ch.name + "**? All messages will be lost permanently.",
    async () => {
      await ch.delete("Deleted by " + msg.author.tag);
      if (ch.id !== msg.channel.id) sendOk(msg.channel, "## ✅ Channel Deleted\n**#" + ch.name + "** has been deleted.");
    }
  );
});

reg("Moderation", "timeout2", ["tempban","softtime","mute2"], "Timeout multiple members at once", ".timeout2 <minutes> @user1 @user2 ...", async (msg, args) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ModerateMembers)) return sendErr(msg.channel, "Missing ModerateMembers permission.");
  const mins    = parseInt(args[0]);
  const targets = [...msg.mentions.members.values()];
  if (isNaN(mins) || mins < 1 || !targets.length) return sendErr(msg.channel, "Usage: " + PREFIX + "timeout2 <minutes> @user1 @user2");
  for (const t of targets) if (t.moderatable) await t.timeout(mins * 60000).catch(() => {});
  sendOk(msg.channel, "## ⏱️ Timed Out\nTimedout **" + targets.length + "** member(s) for **" + mins + "m**.");
});

reg("Moderation", "massdm", ["dm","senddm","bulkdm"], "DM all members with a role (owner only)", ".massdm @role <message>", async (msg, args) => {
  if (msg.author.id !== OWNER_ID) return;
  const role = msg.mentions.roles.first(), text = args.slice(1).join(" ");
  if (!role || !text) return sendErr(msg.channel, "Usage: " + PREFIX + "massdm @role <message>");
  sendConfirm(msg,
    "DM all **" + role.members.size + "** members in **" + role.name + "**?\nMessage: " + text,
    async () => {
      let sent = 0;
      for (const [, m] of role.members) {
        const c = mc(0x5865f2); c.addTextDisplayComponents(tx("## 📩 Message from **" + msg.guild.name + "**\n" + text));
        await m.send({ components: [c], flags: F }).then(() => sent++).catch(() => {});
      }
      sendOk(msg.channel, "## ✅ Mass DM Sent\nDelivered to **" + sent + "** member(s).");
    }
  );
});
...
reg("AFK", "afklist", ["afkall","listafk"], "Show everyone currently AFK", ".afklist", async (msg) => {
  ...
  sendPaged(msg.channel, msg.author.id, list, 10,
    ([id, d]) => "<@" + id + "> — " + d.reason + " (" + fmtDur(Date.now()-d.since) + " ago)",
    "💤 AFK Members (" + list.length + ")"
  );
});
...
reg("AFK", "unafk", ["removeafk","clearafk"], "Remove someone's AFK status (mod only)", ".unafk @user", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ModerateMembers)) return sendErr(msg.channel, "Missing ModerateMembers permission.");
  const target = msg.mentions.users.first();
  if (!target) return sendErr(msg.channel, "Usage: " + PREFIX + "unafk @user");
...
reg("Snipe", "bulksnipe", ["bs","multisnipe"], "Show the last 5 deleted messages", ".bulksnipe", async (msg) => {
  const d = snipe.get(msg.channel.id);
  if (!d) return sendErr(msg.channel, "Nothing to snipe in this channel.");
  const history = d.history ?? [d];
  const c = mc();
  c.addTextDisplayComponents(tx("## 🔍 Bulk Snipe (" + history.length + ")"));
  c.addSeparatorComponents(sp());
  c.addTextDisplayComponents(tx(history.slice(0,5).map((s,i) =>
    "**" + (i+1) + ".** " + s.author + " — <t:" + Math.floor(s.at/1000) + ":R>\n" + (s.content||"*(no text)*")
  ).join("\n\n")));
  send(msg.channel, c);
});

reg("Reminders", "remindall", ["remindchannel","rc3"], "Set a reminder for the whole channel", ".remindall <time> <message>", async (msg, args) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageMessages)) return sendErr(msg.channel, "Missing ManageMessages permission.");
  const ms = parseTime(args[0]), text = args.slice(1).join(" ");
  if (!ms || !text) return sendErr(msg.channel, "Usage: " + PREFIX + "remindall 1h server event starting!");
  const fireAt = Date.now() + ms;
  reminders.push({ id: "channel-" + msg.channel.id + "-" + fireAt, userId: null, channelId: msg.channel.id, message: text, fireAt, everyone: true });
  const c = mc(0x57f287);
  c.addTextDisplayComponents(tx("## ⏰ Channel Reminder Set\n**Message:** " + text + "\n**Fires:** <t:" + Math.floor(fireAt/1000) + ":R>"));
  send(msg.channel, c);
});
...
reg("Fun", "riddle", ["puzzle","brain","rd"], "Get a random riddle", ".riddle", async (msg) => {
  const riddles = [
    ["I speak without a mouth and hear without ears. What am I?", "An echo"],
    ["The more you take, the more you leave behind. What am I?", "Footsteps"],
    ["I have cities, but no houses live there. Mountains, but no trees grow. Water, but no fish swim. What am I?", "A map"],
...
reg("General", "sharedservers", ["mutual","mutualservers","ss2"], "Show servers the bot shares with a user", ".sharedservers @user", async (msg) => {
  const target = msg.mentions.users.first();
  if (!target) return sendErr(msg.channel, "Usage: " + PREFIX + "sharedservers @user");
  const shared = client.guilds.cache.filter(g => g.members.cache.has(target.id));
  const c = mc();
  c.addTextDisplayComponents(tx("## 🌐 Shared Servers with " + target.username + " (" + shared.size + ")"));
  c.addSeparatorComponents(sp());
  c.addTextDisplayComponents(tx(shared.size ? shared.map(g => "**" + g.name + "** (" + g.memberCount + " members)").join("\n") : "No shared servers."));
  send(msg.channel, c);
});

reg("General", "commandstats", ["cs","cmdstats","stats2"], "Show total command and alias count", ".commandstats", async (msg) => {
  const total   = CATEGORIES.reduce((t, cat) => t + cat.list.length, 0);
...
reg("Moderation", "massunrole", ["mur","removeallrole"], "Remove a role from every member", ".massunrole @role", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageRoles, PermissionFlagsBits.Administrator))
    return sendErr(msg.channel, "You need Administrator permission for this.");
  const role = msg.mentions.roles.first();
  if (!role) return sendErr(msg.channel, "Usage: " + PREFIX + "massunrole @role");
  await msg.guild.members.fetch();
  const members = msg.guild.members.cache.filter(m => !m.user.bot && m.roles.cache.has(role.id));
  sendConfirm(msg,
    "Remove " + role + " from **" + members.size + "** members?",
    async () => {
      let done = 0;
      for (const [, m] of members) { await m.roles.remove(role).catch(() => {}); done++; }
      sendOk(msg.channel, "## ✅ Mass Unrole Done\nRemoved " + role + " from **" + done + "** members.");
    }
  );
});

reg("Moderation", "setname", ["botname","renameme","sn2"], "Change the bot's nickname in this server", ".setname <name>", async (msg, args) => {
  if (msg.author.id !== OWNER_ID) return;
  const name = args.join(" ");
  if (!name) return sendErr(msg.channel, "Usage: " + PREFIX + "setname <name>");
  await msg.guild.members.me.setNickname(name);
  sendOk(msg.channel, "## ✅ Nickname Set\nNow displaying as **" + name + "** in this server.");
});

reg("Moderation", "emojidel", ["deleteemoji","removeemoji","delemoji"], "Delete a server emoji", ".emojidel <emoji>", async (msg, args) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageEmojisAndStickers))
    return sendErr(msg.channel, "Missing ManageEmojisAndStickers permission.");
  const match = args[0]?.match(/^<a?:(\w+):(\d+)>$/);
  if (!match) return sendErr(msg.channel, "Usage: " + PREFIX + "emojidel <server emoji>");
  const emoji = msg.guild.emojis.cache.get(match[2]);
  if (!emoji) return sendErr(msg.channel, "That emoji is not from this server.");
  sendConfirm(msg,
    "Delete the emoji **:" + emoji.name + ":**?",
    async () => {
      await emoji.delete();
      sendOk(msg.channel, "## ✅ Emoji Deleted\n**:" + emoji.name + ":** has been removed.");
    }
  );
});

reg("Moderation", "rolemove", ["setrolepos","moverole","rpos"], "Move a role to a specific position", ".rolemove @role <position>", async (msg, args) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageRoles)) return sendErr(msg.channel, "Missing ManageRoles permission.");
  const role = msg.mentions.roles.first(), pos = parseInt(args[1]);
  if (!role || isNaN(pos) || pos < 1) return sendErr(msg.channel, "Usage: " + PREFIX + "rolemove @role <position>");
  await role.setPosition(pos);
  sendOk(msg.channel, "## ✅ Role Moved\n**" + role.name + "** moved to position **" + pos + "**.");
});

reg("Moderation", "cloneserver", ["clonerole","copyrole","cr4"], "Clone a role's permissions to another role", ".cloneserver @from @to", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageRoles)) return sendErr(msg.channel, "Missing ManageRoles permission.");
  const roles = [...msg.mentions.roles.values()];
  if (roles.length < 2) return sendErr(msg.channel, "Usage: " + PREFIX + "cloneserver @from-role @to-role");
  const [from, to] = roles;
  await to.setPermissions(from.permissions);
  sendOk(msg.channel, "## ✅ Permissions Cloned\nCopied **" + from.name + "** permissions to **" + to.name + "**.");
});

reg("Snipe", "clearsnipe", ["cs2","delsnipe","wipesnipe"], "Clear the snipe cache for this channel", ".clearsnipe", async (msg) => {
  if (!hasPerm(msg.member, PermissionFlagsBits.ManageMessages)) return sendErr(msg.channel, "Missing ManageMessages permission.");
  snipe.delete(msg.channel.id);
  esnipe.delete(msg.channel.id);
  sendOk(msg.channel, "## ✅ Snipe Cleared\nSnipe cache cleared for " + msg.channel + ".");
});
...
reg("General", "snowflake", ["sf","parseid","parseflake"], "Parse a Discord snowflake ID", ".snowflake <id>", async (msg, args) => {
  const id = args[0];
  if (!id || isNaN(id)) return sendErr(msg.channel, "Usage: " + PREFIX + "snowflake <discord ID>");
...
reg("Music", "play", ["p","add"], "Play a song or add it to the queue", ".play <song or URL>", async (msg, args) => {
  ...
  try {
    const { track } = await player.play(msg.member.voice.channel, query, {
      nodeOptions: { metadata: { channel: msg.channel, requestedBy: msg.author }, volume: 80 },
    });
    const upd = mc(0x1db954);
    upd.addTextDisplayComponents(tx("## ✅ Added to Queue\n**" + track.title + "**\n" + track.author + "  ·  " + track.duration + "\n-# Requested by " + msg.author.username));
    sent.edit({ components: [upd], flags: F });
  } catch (e) {
    console.error("[PLAY]", e);
    sent.edit({ components: [(() => { const c = mc(0xed4245); c.addTextDisplayComponents(tx("❌  Could not play that track. Try a different search or URL.")); return c; })()], flags: F });
  }
});

reg("Music", "skip", ["s","next"], "Skip the current track", ".skip", async (msg) => {
  const queue = player.nodes.get(msg.guild.id);
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
...
reg("Music", "stop", ["leave","disconnect2","dc3"], "Stop music and clear the queue", ".stop", async (msg) => {
  const queue = player.nodes.get(msg.guild.id);
...
reg("Music", "pause", ["paus"], "Pause the current track", ".pause", async (msg) => {
  ...
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  if (queue.node.isPaused()) return sendErr(msg.channel, "Already paused. Use " + PREFIX + "resume.");
  queue.node.pause();
...
reg("Music", "resume", ["res","unpause"], "Resume the paused track", ".resume", async (msg) => {
  ...
  if (!queue) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  if (!queue.node.isPaused()) return sendErr(msg.channel, "Not paused. Use " + PREFIX + "pause.");
  queue.node.resume();
...
reg("Music", "nowplaying", ["np","current","playing"], "Show the currently playing track", ".nowplaying", async (msg) => {
  ...
  c.addSeparatorComponents(sp());
  c.addTextDisplayComponents(tx(progress + "\n\n**Volume:** " + queue.node.volume + "%  ·  **Loop:** " + (queue.repeatMode === 0 ? "Off" : queue.repeatMode === 1 ? "Track" : "Queue") + "\n-# Requested by " + (track.requestedBy?.username ?? "Unknown")));
  send(msg.channel, c);
});
...
reg("Music", "queue", ["q2","list","tracklist"], "Show the current queue", ".queue [page]", async (msg, args) => {
  ...
  const c = mc(0x1db954);
  c.addTextDisplayComponents(tx("## 🎶 Queue  ·  " + tracks.length + " track(s)"));
  c.addSeparatorComponents(sp());
  c.addTextDisplayComponents(tx("**Now Playing:** " + current.title + "  ·  " + current.duration));
  if (tracks.length) {
    ...
    c.addSeparatorComponents(sp());
    c.addTextDisplayComponents(tx(
      slice.map((t, i) => "**" + (page * perPage + i + 1) + ".** " + t.title + "  ·  " + t.duration).join("\n") +
      (tracks.length > perPage ? "\n\n-# Page " + (page + 1) + " of " + Math.ceil(tracks.length / perPage) : "")
    ));
  }
  send(msg.channel, c);
});
...
reg("Music", "shuffle", ["mix","shuf"], "Shuffle the queue", ".shuffle", async (msg) => {
  ...
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  if (!queue.tracks.size) return sendErr(msg.channel, "Not enough tracks in the queue to shuffle.");
  queue.tracks.shuffle();
...
reg("Music", "loop", ["repeat","lp"], "Set loop mode (off/track/queue)", ".loop <off|track|queue>", async (msg, args) => {
  ...
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  const modes = { off: QueueRepeatMode.OFF, track: QueueRepeatMode.TRACK, queue: QueueRepeatMode.QUEUE, autoplay: QueueRepeatMode.AUTOPLAY };
  const mode  = args[0]?.toLowerCase();
  if (!mode || !(mode in modes)) return sendErr(msg.channel, "Usage: " + PREFIX + "loop off / track / queue / autoplay");
  queue.setRepeatMode(modes[mode]);
  const labels = { off: "Off", track: "Track 🔂", queue: "Queue 🔁", autoplay: "Autoplay ♾️" };
  sendOk(msg.channel, "## 🔁 Loop\nMode set to **" + labels[mode] + "**");
});

reg("Music", "remove", ["rem2","rmtrack","deltrack"], "Remove a track from the queue by position", ".remove <position>", async (msg, args) => {
  const queue = player.nodes.get(msg.guild.id);
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  const pos = parseInt(args[0]) - 1;
  if (isNaN(pos) || pos < 0 || pos >= queue.tracks.size) return sendErr(msg.channel, "Usage: " + PREFIX + "remove <position>  —  see " + PREFIX + "queue for positions");
  const track = queue.tracks.toArray()[pos];
  queue.node.remove(pos);
  sendOk(msg.channel, "## 🗑️ Removed\n**" + track.title + "** removed from the queue.");
});

reg("Music", "clearqueue", ["cq","emptyqueue","clearq"], "Clear all tracks from the queue", ".clearqueue", async (msg) => {
  const queue = player.nodes.get(msg.guild.id);
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  queue.tracks.clear();
...
reg("Music", "seek", ["sk2","seekto"], "Seek to a position in the current track", ".seek <time>  e.g. 1:30", async (msg, args) => {
  const queue = player.nodes.get(msg.guild.id);
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  const raw = args[0];
  if (!raw) return sendErr(msg.channel, "Usage: " + PREFIX + "seek 1:30");
...
reg("Music", "skipto", ["st2","jumpqueue","jump"], "Skip to a specific track in the queue", ".skipto <position>", async (msg, args) => {
  const queue = player.nodes.get(msg.guild.id);
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  const pos = parseInt(args[0]) - 1;
  if (isNaN(pos) || pos < 0 || pos >= queue.tracks.size) return sendErr(msg.channel, "Usage: " + PREFIX + "skipto <position>");
  queue.node.skipTo(pos);
...
reg("Music", "previous", ["prev","back"], "Play the previous track", ".previous", async (msg) => {
  ...
  try {
    await queue.history.previous();
    sendOk(msg.channel, "## ⏮️ Playing previous track");
  } catch { sendErr(msg.channel, "No previous track in history."); }
});
...
client.on("messageCreate", async (msg) => {
  ...
  const lacking = needs.filter(p => botPerms && !botPerms.has(p))
    .map(p => Object.entries(PermissionFlagsBits).find(([, v]) => v === p)?.[0]?.replace(/([A-Z])/g, " $1").trim());
  if (lacking.length) {
    const c = mc(0xed4245);
    c.addTextDisplayComponents(tx("## ❌ Missing Bot Permissions\nI need the following permissions to run this command:\n" + lacking.map(p => "• " + p).join("\n")));
    return send(msg.channel, c);
  }

  try { await command.run(msg, args, name); }
  catch (e) {
