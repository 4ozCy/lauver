...
function buildNowPlaying(player, track) {
  ...
  c.addTextDisplayComponents(tx(
    ...
    "  ·  "+(loopLabels[player.loop]??"🔄 Off")+"  ·  🤖 "+(autoplay?"Autoplay On":"Autoplay Off")+"  ·  🔊 "+player.volume+"%"
  ));
  ...
  c.addActionRowComponents(new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId("music:autoplay").setLabel("🤖  Autoplay").setStyle(autoplay?ButtonStyle.Success:ButtonStyle.Secondary),
...
function _buildSmartAutoplayQuery(track) {
  const source = String(_firstDefined(track, ["sourceName","info.sourceName","track.info.sourceName","pluginInfo.sourceName"]) || "").toLowerCase();
  const identifier = _firstDefined(track, ["identifier","info.identifier","track.info.identifier","encoded","info.encoded"]);
  const uri = _firstDefined(track, ["uri","info.uri","info.url","track.info.uri","track.info.url"]);
  const isrc = _firstDefined(track, ["isrc","info.isrc","pluginInfo.isrc","pluginInfo.extra.isrc","extra.isrc","info.extra.isrc"]);

  // Spotify recommendations can be seeded by ISRC (best) or Spotify track id.
  // LavaSrc supported queries: sprec:mix:track:<ID> and sprec:mix:isrc:<ISRC>
  ...
  // Other LavaSrc recommendation queries (if the track source supports them)
  if (source.includes("deezer")) {
    const m = String(uri || "").match(/\/track\/(\d+)/);
    const did = m?.[1] || identifier;
    if (did) return `dzrec:${did}`;
  }
  if (source.includes("yandex")) { if (identifier) return `ymrec:${identifier}`; }
  if (source.includes("vk"))     { if (identifier) return `vkrec:${identifier}`; }
  if (source.includes("tidal"))  { if (identifier) return `tdrec:${identifier}`; }
  if (source.includes("qobuz"))  { if (identifier) return `qbrec:${identifier}`; }
  if (source.includes("jiosaavn")) { if (identifier) return `jsrec:${identifier}`; }

  // Fallback: keyword search (works everywhere)
...
// --- Autoplay genre-ish helper (heuristic, no extra Lavalink plugins needed) ---
const _GENRE_HINTS = [
  // electronic
  "edm","electronic","dance","house","deep house","tech house","techno","trance","progressive","hardstyle",
  "dubstep","drum and bass","dnb","drum & bass","future bass","bass","electro","synthwave","retrowave",
  // hiphop / rap
  "hip hop","hiphop","rap","trap","drill","boom bap","lo-fi","lofi","phonk",
  // pop / rock / metal
  "pop","indie","rock","alt","alternative","punk","metal","emo",
  // chill / ambient
  "chill","chillhop","ambient","sleep","study","focus","relax","instrumental",
  // misc styles
  "remix","nightcore","acoustic","piano","guitar","cover","live"
];
...
function _extractGenreHints(track) {
  ...
  // if we found nothing, we still keep a small "style" anchor from title keywords
  // (helps keep it in the same *vibe* even without explicit genre words)
  if (found.size === 0) {
    const vibe = hay.split(" ").filter(w => w.length >= 5).slice(0, 3);
    for (const w of vibe) found.add(w);
  }

  return Array.from(found).slice(0, 4); // keep it short
}
...
kazagumo.on("playerEmpty", async (player) => {
  ...
  if (autoplay && last) {
    // Autoplay (genre-ish): use hints from the last song (lofi/phonk/house/etc.) and pick a DIFFERENT track that matches the same vibe.
    const hints = _extractGenreHints(last);
    const q = `ytsearch:${String(`${hints.join(" ")} ${last.author ?? ""} ${last.title ?? ""}`).trim()}`;
    const res = await kazagumo.search(q, { requester: last.requester ?? client.user }).catch(() => null);
    const tracks = res?.tracks ?? [];

    // keep a short recent memory so we don't loop the same few tracks
    const recent = Array.isArray(player.data.get("autoplayRecent")) ? player.data.get("autoplayRecent") : [];
    const seen = new Set(recent);

    const next =
      tracks.find(t => t?.uri && t.uri !== last?.uri && _sharesHints(t, hints) && !seen.has(t.uri)) ||
      tracks.find(t => t?.uri && t.uri !== last?.uri && _sharesHints(t, hints)) ||
      tracks.find(t => t?.uri && t.uri !== last?.uri) ||
      tracks[0];
...
Added **" + next.title + "**"));
        const m = await channel.send({ components: [c], flags: F, allowedMentions: { parse: [] } }).catch(() => null);
  ...
  // clear VC status when queue ends (and autoplay is off / failed)
...
No more tracks."));
  channel?.send({ components: [c], flags: F, allowedMentions: { parse: [] } });
});
kazagumo.on("playerClosed", (player) => {
  // clear VC status when player disconnects/destroys
  void _setVoiceChannelStatus(_getPlayerVoiceChannelId(player), "");
  const old = player.data.get("npMessage");
  if (old) { old.delete().catch(() => {}); player.data.delete("npMessage"); }
...
reg("Music","play",["p","add"],"Play a song or add it to the queue",".play <song or URL>", async (msg,args) => {
  ...
  try {
    ...
    if(res.type==="PLAYLIST"){
      ...
      await sent.edit({components:[u],flags:F});
...
client.on("interactionCreate", async (interaction) => {
  ...
  if(interaction.isButton()){
    ...
    if(id.startsWith("music:")){
      ...
      if(action==="volume"){
        const modal = buildModal("modal:musicvol:"+interaction.user.id,"Set Volume",
          input("volume","Volume (1–100)",TextInputStyle.Short,{placeholder:String(player.volume??80),max:3})
        );
...
async function handleMusicSlash(interaction) {
  ...
  if(name==="play"){
    ...
    const m = await interaction.editReply({components:[c],flags:F,allowedMentions:{parse:[]}}).catch(()=>null);
    if(m) setTimeout(()=>m.delete().catch(()=>{}),2000);
  }
