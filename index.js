...
function buildNowPlaying(queue, track) {
  const loopLabel = ["🔄 Off", "🔂 Track", "🔁 Queue", "♾️ Auto"][queue.repeatMode] ?? "🔄 Off";
  ...
  c.addTextDisplayComponents(tx(
    ...
    "  ·  Queue: " + (queue.tracks.size + 1) + "  ·  Loop: " + loopLabel +
...
reg("Music", "play", ["p","add"], "Play a song or add it to the queue", ".play <song or URL>", async (msg, args) => {
  ...
  try {
    ...
    const upd = mc(0xed4245); upd.addTextDisplayComponents(tx("❌  Could not play that. Try a different search or URL."));
    sent.edit({ components: [upd], flags: F });
  }
});
...
reg("Music", "queue", ["q2","list","tracklist"], "Show the current queue", ".queue [page]", async (msg, args) => {
  ...
  if (tracks.length) {
    ...
    c.addSeparatorComponents(sp());
    c.addTextDisplayComponents(tx(slice.map((t, i) => "**" + (page * 10 + i + 1) + ".** " + t.title + "  ·  " + t.duration).join("\n")));
  }
  send(msg.channel, c);
});
...
reg("Music", "shuffle", ["mix","shuf"], "Shuffle the queue", ".shuffle", async (msg) => {
  ...
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  if (queue.tracks.size < 2) return sendErr(msg.channel, "Not enough tracks to shuffle.");
  queue.tracks.shuffle();
  sendOk(msg.channel, "## 🔀 Shuffled\nQueue has been shuffled!");
});

reg("Music", "loop", ["repeat","lp"], "Set loop mode (off/track/queue/autoplay)", ".loop <off|track|queue|autoplay>", async (msg, args) => {
  const queue = player.nodes.get(msg.guild.id);
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
...
reg("Music", "remove", ["rem","rmtrack"], "Remove a track from the queue by position", ".remove <position>", async (msg, args) => {
  ...
  if (isNaN(pos) || pos < 0 || pos >= queue.tracks.size) return sendErr(msg.channel, "Invalid position. See " + PREFIX + "queue.");
...
reg("Music", "previous", ["prev","back"], "Play the previous track", ".previous", async (msg) => {
  ...
  if (!queue?.isPlaying()) return sendErr(msg.channel, "Nothing is playing right now.");
  if (!msg.member.voice.channel) return sendErr(msg.channel, "You need to be in a voice channel.");
  try { await queue.history.previous(); sendOk(msg.channel, "## ⏮️ Playing previous track"); }
...
async function handleButton(interaction) {
  ...
  if (id.startsWith("music:")) {
    ...
    if (action === "skip")       { queue.node.skip(); return interaction.deferUpdate(); }
    if (action === "stop")       { queue.delete(); await interaction.deferUpdate(); const c = mc(0xed4245); c.addTextDisplayComponents(tx("## ⏹️ Stopped")); return interaction.message.edit({ components: [c], flags: F }); }
    if (action === "playpause")  { queue.node.isPaused() ? queue.node.resume() : queue.node.pause(); await interaction.deferUpdate(); return interaction.message.edit(buildNowPlaying(queue, queue.currentTrack)); }
    if (action === "shuffle")    { if (queue.tracks.size < 2) { const c = mc(0xed4245); c.addTextDisplayComponents(tx("❌  Not enough tracks.")); return interaction.reply(eph(c)); } queue.tracks.shuffle(); await interaction.deferUpdate(); return interaction.message.edit(buildNowPlaying(queue, queue.currentTrack)); }
    if (action === "loop")       { queue.setRepeatMode((queue.repeatMode + 1) % 4); await interaction.deferUpdate(); return interaction.message.edit(buildNowPlaying(queue, queue.currentTrack)); }
    if (action === "clearqueue") { queue.tracks.clear(); await interaction.deferUpdate(); return interaction.message.edit(buildNowPlaying(queue, queue.currentTrack)); }
    if (action === "prev")       {
      try { await queue.history.previous(); return interaction.deferUpdate(); }
      catch { const c = mc(0xed4245); c.addTextDisplayComponents(tx("❌  No previous track.")); return interaction.reply(eph(c)); }
    }
    if (action === "volume") {
      return interaction.showModal(buildModal("modal:volume:" + interaction.message.id, "Set Volume",
        input("volume", "Volume (1–100)", TextInputStyle.Short, { placeholder: "Enter 1–100", value: String(queue.node.volume), max: 3, min: 1 })
      ));
    }
    return;
  }
...
async function handleModal(interaction) {
  ...
  if (type === "volume") {
    ...
    const queue = player.nodes.get(interaction.guild.id);
    if (!queue?.isPlaying()) { const c = mc(0xed4245); c.addTextDisplayComponents(tx("❌  Nothing is playing.")); return interaction.reply(eph(c)); }
    queue.node.setVolume(vol);
