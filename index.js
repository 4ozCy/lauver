...
async function cleanupPlayerUI(player, reasonText = null) {
  ...
  if (reasonText) {
    ...
    if (channel) {
      ...
      channel.send({ components: [c], flags: F, allowedMentions: { parse: [] } }).catch(() => {});
    }
  }
}
...
client.on("interactionCreate", async (interaction) => {
  ...
  if(interaction.isAutocomplete()){
    ...
    try {
      const res = await kazagumo.search(focused, { requester: interaction.user }).catch(() => null);
...
client.once("clientReady", async () => {
  ...
  const musicSlash = [
    new SlashCommandBuilder().setName("play").setDescription("Play a song or add it to the queue").addStringOption(o=>o.setName("query").setDescription("Song name or URL").setRequired(true).setAutocomplete(true)),
