...
const client = new Client(
{
	intents: [
		...
		GatewayIntentBits.GuildModeration, GatewayIntentBits.GuildVoiceStates, GatewayIntentBits.GuildMessageReactions, GatewayIntentBits.GuildPresences,
	],
...
{
	return new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("confirm:" + key).setLabel("Confirm").setStyle(ButtonStyle.Danger), new ButtonBuilder().setCustomId("cancel:" + key).setLabel("Cancel").setStyle(ButtonStyle.Secondary));
}
...
async function renderPage(channel, id, page, editMsg = null)
{
	...
	const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("pg:prev:" + id + ":" + page).setLabel("← Prev").setStyle(ButtonStyle.Secondary).setDisabled(page === 0), new ButtonBuilder().setCustomId("pg:next:" + id + ":" + page).setLabel("Next →").setStyle(ButtonStyle.Secondary).setDisabled(page >= p.total - 1));
...
{
	const candidates = [
		track?.thumbnail, track?.info?.thumbnail, track?.artworkUrl, track?.info?.artworkUrl, track?.pluginInfo?.artworkUrl, track?.pluginInfo?.thumbnail,
	];
	return candidates.find((url) => typeof url === "string" && /^https?:\/\//.test(url)) || null;
}
async function cleanupPlayerUI(player, reasonText = null)
{
	if(!player) return;
	...
	if(reasonText)
	{
		...
		{
			const c = mc(0x5865f2);
			c.addTextDisplayComponents(tx(reasonText));
			channel.send(
			{
				components: [c],
				flags: F,
...
function buildNowPlaying(player, track)
{
	...
	{
		name: "pause",
		id: "1484463220143161454"
	}).setLabel(paused ? "Resume" : "Pause").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId("music:skip").setEmoji(
	{
		name: "skip",
		id: "1484463128501813299"
	}).setLabel("Skip").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId("music:queue").setEmoji(
	{
		name: "queue",
		id: "1484463027083284601"
	}).setLabel("Queue").setStyle(ButtonStyle.Secondary), ));
	c.addActionRowComponents(new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId("music:loop").setEmoji(
	{
		name: "loop",
		id: "1484463176547565700"
	}).setLabel("Loop").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId("music:volume").setEmoji(
	{
		name: "volume",
		id: "1484463072683888741"
	}).setLabel("Volume").setStyle(ButtonStyle.Secondary), new ButtonBuilder().setCustomId("music:stop").setEmoji(
	{
		name: "stop",
		id: "1484463285897269278"
	}).setLabel("End Session").setStyle(ButtonStyle.Secondary), ));
	return {
		components: [c],
		flags: F,
		allowedMentions:
...
{
	await cleanupPlayerUI(player, "## 🎵 Queue Ended\nNo more tracks.");
});
kazagumo.on("playerClosed", async (player) =>
{
	const silentClose = !!player.data?.get("silentClose");
	player.data?.delete("silentClose");
	await cleanupPlayerUI(player, silentClose ? null : "## 🔌 Player disconnected.");
});
...
const DISABLED_CATEGORIES = new Set(["Tools", "Snipe", "Reminders"]);
const CATEGORIES = [
{
	icon: "🌐",
	name: "General",
...
{
	if(DISABLED_CATEGORIES.has(CATEGORIES[i].name)) CATEGORIES.splice(i, 1);
}

function reg(cat, name, aliases, desc, usage, slashOrRun, maybeRun)
{
	if(DISABLED_CATEGORIES.has(cat)) return;
	let slashFn = null,
		run;
...
async function dispatchSlash(interaction)
{
	...
	const shim = {
		...
		channel: new Proxy(interaction.channel,
		{
			get(target, prop)
			{
				if(prop === "send") return async (payload) => interaction.editReply(payload).catch(
...
function buildHelpCat(cat)
{
	...
	const select = new StringSelectMenuBuilder().setCustomId("help:cat").setPlaceholder("Browse a category...").addOptions(CATEGORIES.filter(cat => cat.list.length).map(cat => new StringSelectMenuOptionBuilder().setLabel(cat.name).setValue(cat.name).setDescription(cat.list.length + " commands").setEmoji(cat.icon)));
	return {
		container: c,
...
reg("General", "userinfo", ["ui", "whois", "who"], "User profile info", ".userinfo [@user]", cmd => cmd.addUserOption(o => o.setName("user").setDescription("Target user").setRequired(false)), async (msg, args) =>
{
...
reg("General", "avatar", ["av", "pfp", "pic"], "Get a user's avatar", ".avatar [@user]", cmd => cmd.addUserOption(o => o.setName("user").setDescription("Target user").setRequired(false)), async (msg) =>
{
	const target = msg.mentions?.users?.first() ?? (msg._interaction?.options?.getUser("user")) ?? msg.author;
...
reg("General", "banner", ["userbanner"], "Get a user's banner", ".banner [@user]", cmd => cmd.addUserOption(o => o.setName("user").setDescription("Target user").setRequired(false)), async (msg) =>
{
	const target = msg.mentions?.users?.first() ?? (msg._interaction?.options?.getUser("user")) ?? msg.author;
...
reg("General", "roleinfo", ["ri"], "Info about a role", ".roleinfo @role", cmd => cmd.addRoleOption(o => o.setName("role").setDescription("Target role").setRequired(true)), async (msg) =>
{
...
reg("General", "channelinfo", ["ci"], "Info about a channel", ".channelinfo [#channel]", cmd => cmd.addChannelOption(o => o.setName("channel").setDescription("Target channel").setRequired(false)), async (msg) =>
{
...
reg("General", "inrole", ["ir"], "List members with a role", ".inrole @role", cmd => cmd.addRoleOption(o => o.setName("role").setDescription("Target role").setRequired(true)), async (msg) =>
{
	const role = msg.mentions?.roles?.first() ?? (msg._interaction?.options?.getRole("role"));
...
reg("General", "permissions", ["perms", "perm"], "List a user's permissions", ".permissions [@user]", cmd => cmd.addUserOption(o => o.setName("user").setDescription("Target user").setRequired(false)), async (msg) =>
{
...
reg("General", "joinpos", ["jp", "joinrank"], "Show member join position", ".joinpos [@user]", cmd => cmd.addUserOption(o => o.setName("user").setDescription("Target user").setRequired(false)), async (msg) =>
{
	if(!msg.guild) return;
	await msg.guild.members.fetch();
	const target = msg.mentions?.members?.first() ?? (msg._interaction?.options?.getMember("user")) ?? msg.member;
...
reg("General", "presence", ["activity", "doing"], "See what a user is doing", ".presence [@user]", cmd => cmd.addUserOption(o => o.setName("user").setDescription("Target user").setRequired(false)), async (msg) =>
{
	const target = msg.mentions?.members?.first() ?? (msg._interaction?.options?.getMember("user")) ?? msg.member;
...
reg("General", "created", ["age", "accountage"], "Show when an account was created", ".created [@user]", cmd => cmd.addUserOption(o => o.setName("user").setDescription("Target user").setRequired(false)), async (msg) =>
{
	const target = msg.mentions?.users?.first() ?? (msg._interaction?.options?.getUser("user")) ?? msg.author;
	const c = mc();
...
reg("General", "lookup", ["fetchuser", "getuser"], "Fetch any user by ID", ".lookup <userID>", cmd => cmd.addStringOption(o => o.setName("id").setDescription("User ID to look up").setRequired(true)), async (msg, args) =>
{
...
reg("General", "snowflake", ["sf", "parseid"], "Parse a Discord snowflake ID", ".snowflake <id>", cmd => cmd.addStringOption(o => o.setName("id").setDescription("Snowflake ID").setRequired(true)), async (msg, args) =>
{
	const id = msg._interaction?.options?.getString("id") || args[0];
...
reg("General", "help", ["h", "commands", "cmds"], "List commands or get details on one", ".help [command]", cmd => cmd.addStringOption(o => o.setName("command").setDescription("Command or category name").setRequired(false)), async (msg, args) =>
{
...
reg("Music", "stop", ["leave", "dc"], "Stop music and leave the voice channel", ".stop", async (msg) =>
{
	...
	if(!msg.member.voice.channel) return _sendErr(msg.channel, "You need to be in a voice channel.");
	q.data.set("silentClose", true);
	await cleanupPlayerUI(q);
	q.destroy();
...
reg("Music", "lyrics", ["ly", "lyric", "song"], "Get song lyrics", ".lyrics <artist> - <title>", async (msg, args) =>
{
	...
	try
	{
		data = await fetchJSON("https://api.lyrics.ovh/v1/" + encodeURIComponent(artist) + "/" + encodeURIComponent(title));
	}
	catch
	{
...
reg("Tools", "weather", ["wthr", "forecast"], "Get weather for a city", ".weather <city>", cmd => cmd.addStringOption(o => o.setName("city").setDescription("City name").setRequired(true)), async (msg, args) =>
{
...
reg("Tools", "define", ["def", "dictionary", "dict"], "Look up a word definition", ".define <word>", cmd => cmd.addStringOption(o => o.setName("word").setDescription("Word to define").setRequired(true)), async (msg, args) =>
{
	...
	try
	{
		data = await fetchJSON("https://api.dictionaryapi.dev/api/v2/entries/en/" + encodeURIComponent(word.toLowerCase()));
	}
	catch
	{
...
reg("Tools", "urban", ["ud", "urb", "slang"], "Look up a term on Urban Dictionary", ".urban <term>", cmd => cmd.addStringOption(o => o.setName("term").setDescription("Term to look up").setRequired(true)), async (msg, args) =>
{
...
reg("Tools", "qr", ["qrcode", "qrgen"], "Generate a QR code", ".qr <text>", cmd => cmd.addStringOption(o => o.setName("text").setDescription("Text or URL").setRequired(true)), async (msg, args) =>
{
...
reg("Tools", "currency", ["convert", "fx", "exchange"], "Convert currency", ".currency <amount> <from> <to>", cmd => cmd.addNumberOption(o => o.setName("amount").setDescription("Amount").setRequired(true)).addStringOption(o => o.setName("from").setDescription("From currency (e.g. USD)").setRequired(true)).addStringOption(o => o.setName("to").setDescription("To currency (e.g. EUR)").setRequired(true)), async (msg, args) =>
{
...
reg("Fun", "dice", ["multidice"], "Roll multiple dice (e.g. 2d6)", ".dice <NdS>", cmd => cmd.addStringOption(o => o.setName("notation").setDescription("e.g. 2d6").setRequired(true)), async (msg, args) =>
{
...
reg("Fun", "8ball", ["8b", "ball", "magic"], "Magic 8-ball", ".8ball <question>", cmd => cmd.addStringOption(o => o.setName("question").setDescription("Your question").setRequired(true)), async (msg, args) =>
{
...
reg("Fun", "choose", ["pick", "pickone"], "Pick randomly from a list", ".choose a, b, c", cmd => cmd.addStringOption(o => o.setName("options").setDescription("Comma-separated options").setRequired(true)), async (msg, args) =>
{
...
reg("Fun", "calculate", ["calc", "math"], "Math expression evaluator", ".calculate <expression>", cmd => cmd.addStringOption(o => o.setName("expression").setDescription("Math expression").setRequired(true)), async (msg, args) =>
{
...
reg("Fun", "random", ["rand", "rng"], "Random number in a range", ".random <min> <max>", cmd => cmd.addIntegerOption(o => o.setName("min").setDescription("Minimum").setRequired(true)).addIntegerOption(o => o.setName("max").setDescription("Maximum").setRequired(true)), async (msg, args) =>
{
...
reg("Fun", "rate", ["rateme", "score"], "Rate something out of 10", ".rate <thing>", cmd => cmd.addStringOption(o => o.setName("thing").setDescription("Thing to rate").setRequired(true)), async (msg, args) =>
{
...
reg("Fun", "rps", ["rockpaperscissors"], "Rock paper scissors vs the bot", ".rps <rock|paper|scissors>", cmd => cmd.addStringOption(o => o.setName("choice").setDescription("rock, paper, or scissors").setRequired(true).addChoices(
{
	name: "Rock",
	value: "rock"
},
...
reg("Fun", "horoscope", ["horo", "zodiac", "starsign"], "Get info about a zodiac sign", ".horoscope <sign>", cmd => cmd.addStringOption(o => o.setName("sign").setDescription("Zodiac sign").setRequired(true).addChoices(...Object.keys(ZODIAC).map(k => (
{
...
reg("Fun", "reverse", ["rev"], "Reverse text", ".reverse <text>", cmd => cmd.addStringOption(o => o.setName("text").setDescription("Text to reverse").setRequired(true)), async (msg, args) =>
{
	const text = msg._interaction?.options?.getString("text") || args.join(" ");
...
reg("Fun", "mock", ["spongebob", "sb"], "SpOnGeBoB mock text", ".mock <text>", cmd => cmd.addStringOption(o => o.setName("text").setDescription("Text to mock").setRequired(true)), async (msg, args) =>
{
	const text = msg._interaction?.options?.getString("text") || args.join(" ");
	if(!text) return _sendErr(msg.channel, "Usage: " + PREFIX + "mock <text>");
	const c = mc(0xfee75c);
	c.addTextDisplayComponents(tx("## 🧽 " + text.split("").map((ch, i) => i % 2 ? ch.toUpperCase() : ch.toLowerCase()).join("")));
	_send(msg.channel, c);
});
reg("Fun", "clap", ["claptext"], "Add 👏 between words", ".clap <text>", cmd => cmd.addStringOption(o => o.setName("text").setDescription("Text to clap").setRequired(true)), async (msg, args) =>
{
	const text = msg._interaction?.options?.getString("text") || args.join(" ");
...
reg("Moderation", "purge", ["clear", "bulkdelete", "prune"], "Delete multiple messages", ".purge <amount>", cmd => cmd.addIntegerOption(o => o.setName("amount").setDescription("Number of messages (1-100)").setRequired(true).setMinValue(1).setMaxValue(100)), async (msg, args) =>
{
	if(!hasPerm(msg.member, PermissionFlagsBits.ManageMessages)) return _sendErr(msg.channel, "Missing ManageMessages permission.");
...
reg("Moderation", "slowmode", ["sm", "slow"], "Set slowmode on a channel", ".slowmode <seconds>", cmd => cmd.addIntegerOption(o => o.setName("seconds").setDescription("Slowmode duration in seconds (0 to disable)").setRequired(true).setMinValue(0).setMaxValue(21600)), async (msg, args) =>
{
...
client.on("messageCreate", async (msg) =>
{
	...
	if(afkStore.has(msg.author.id))
	{
		...
		}).then(m => setTimeout(() => m.delete().catch(() =>
		{}), 5000));
	}
...
client.on("interactionCreate", async (interaction) =>
{
	if(interaction.isAutocomplete())
	{
		...
		try
		{
			...
			if(!tracks.length) return interaction.respond([]).catch(() =>
			{});
			const choices = tracks.map((track) =>
			{
				const label = `${track.title} • ${track.author ?? "Unknown"} • ${msToTime(track.length)}`.slice(0, 100);
				const value = `${track.title} ${track.author ?? ""}`.trim().slice(0, 100);
				return {
					name: label,
					value
				};
	...
	if(interaction.isButton())
	{
		...
		if(id.startsWith("openmodal:"))
		{
			...
			if(type === "say") return interaction.showModal(buildModal("modal:say:" + uid + ":" + extra, "Say Something", input("content", "Message", TextInputStyle.Paragraph,
			{
				placeholder: "Type your message here...",
				max: 2000
			})));
			if(type === "announce") return interaction.showModal(buildModal("modal:announce:" + uid + ":" + extra, "Announcement", input("content", "Announcement", TextInputStyle.Paragraph,
			{
				placeholder: "Write your announcement...",
				max: 4000
			})));
			if(type === "poll") return interaction.showModal(buildModal("modal:poll:" + uid + ":" + extra, "Create Poll", input("question", "Poll Question", TextInputStyle.Short,
			{
				placeholder: "What is your question?",
				max: 256
			})));
			if(type === "note") return interaction.showModal(buildModal("modal:note:" + uid, "Add Note", input("text", "Note", TextInputStyle.Paragraph,
			{
				placeholder: "Write your note...",
				max: 1000
			})));
			if(type === "tag") return interaction.showModal(buildModal("modal:tag:" + uid + ":" + extra, "Set Tag: " + extra, input("text", "Tag Content", TextInputStyle.Paragraph,
			{
				placeholder: "Write the tag content...",
				max: 2000
			})));
			if(type === "afk") return interaction.showModal(buildModal("modal:afk:" + uid, "Set AFK Reason", input("reason", "Reason", TextInputStyle.Short,
			{
				placeholder: "Why are you going AFK?",
				max: 200
			})));
			if(type === "remind") return interaction.showModal(buildModal("modal:remind:" + uid + ":" + extra, "Set Reminder", input("duration", "Duration", TextInputStyle.Short,
			{
		...
		if(id.startsWith("music:"))
		{
			...
			if(action === "stop")
			{
				...
				player.data.set("silentClose", true);
				await cleanupPlayerUI(player);
				player.destroy();
			...
			if(action === "queue")
			{
				...
				{
					c.addSeparatorComponents(sp());
					c.addTextDisplayComponents(tx("No tracks are queued."));
				}
			...
			if(action === "volume")
			{
				const modal = buildModal("modal:musicvol:" + interaction.user.id, "Set Volume", input("volume", "Volume (1–100)", TextInputStyle.Short,
				{
...
client.once("clientReady", async () =>
{
	...
	const SLASH_SKIP = new Set([...["slap", "bonk", "punch", "kick", "bite", "yeet", "poke", "tickle", "hug", "pat", "kiss", "cuddle", "feed", "handhold", "highfive", "wave", "handshake", "blush", "smile", "laugh", "cry", "pout", "stare", "shrug", "facepalm", "dance", "wink"], "say", "announce", "poll", "poll2", "stealemoji", "addrole", "removerole", "massrole", "lock", "unlock", "softban", "clearsnipe", ]);
	const seen = new Set();
	const prefixSlash = [];
	...
	const musicSlash = [
		new SlashCommandBuilder().setName("play").setDescription("Play a song or add it to the queue").addStringOption(o => o.setName("query").setDescription("Song name or URL").setRequired(true).setAutocomplete(true)), new SlashCommandBuilder().setName("skip").setDescription("Skip the current track"), new SlashCommandBuilder().setName("stop").setDescription("Stop music and disconnect"),
		new SlashCommandBuilder().setName("pause").setDescription("Pause the current track"), new SlashCommandBuilder().setName("resume").setDescription("Resume the paused track"), new SlashCommandBuilder().setName("nowplaying").setDescription("Show the currently playing track"), new SlashCommandBuilder().setName("queue").setDescription("Show the current queue").addIntegerOption(o => o.setName("page").setDescription("Page number").setRequired(false).setMinValue(1)), new SlashCommandBuilder().setName("volume").setDescription("Set the playback volume").addIntegerOption(o => o.setName("level").setDescription("Volume 1-100").setRequired(true).setMinValue(1).setMaxValue(100)), new SlashCommandBuilder().setName("loop").setDescription("Set loop mode").addStringOption(o => o.setName("mode").setDescription("Loop mode").setRequired(true).addChoices(
		{
			name: "Off",
			value: "none"
		},
		...
		},
		{
			name: "Queue",
			value: "queue"
		})), new SlashCommandBuilder().setName("shuffle").setDescription("Shuffle the queue"), new SlashCommandBuilder().setName("seek").setDescription("Seek to a position in the track").addStringOption(o => o.setName("time").setDescription("Timestamp e.g. 1:30").setRequired(true)), new SlashCommandBuilder().setName("remove").setDescription("Remove a track from the queue").addIntegerOption(o => o.setName("position").setDescription("Queue position").setRequired(true).setMinValue(1)), new SlashCommandBuilder().setName("skipto").setDescription("Jump to a track in the queue").addIntegerOption(o => o.setName("position").setDescription("Queue position").setRequired(true).setMinValue(1)), new SlashCommandBuilder().setName("clearqueue").setDescription("Clear all upcoming tracks"), new SlashCommandBuilder().setName("lyrics").setDescription("Get song lyrics").addStringOption(o => o.setName("artist").setDescription("Artist name").setRequired(true)).addStringOption(o => o.setName("title").setDescription("Song title").setRequired(true)), new SlashCommandBuilder().setName("spotify").setDescription("Show what someone is listening to on Spotify").addUserOption(o => o.setName("user").setDescription("User to check").setRequired(false)), new SlashCommandBuilder().setName("join").setDescription("Join your voice channel"), new SlashCommandBuilder().setName("disconnect").setDescription("Disconnect from the voice channel"), new SlashCommandBuilder().setName("movehere").setDescription("Move the bot to your voice channel"),
	].map(c => c.toJSON());
	...
	try
	{
		...
		console.log("[SLASH] Registered " + all.length + " slash commands (" + musicSlash.length + " music + " + trimmedPrefix.length + " general).");
	}
	catch (e)
	{
