...
function buildNowPlaying(player, track) {
  ...
  c.addActionRowComponents(new ActionRowBuilder().addComponents(
    ...
    new ButtonBuilder().setCustomId("music:volume").setLabel("🔊  Volume").setStyle(ButtonStyle.Secondary),    new ButtonBuilder().setCustomId("music:clearqueue").setLabel("🗑  Clear Queue").setStyle(ButtonStyle.Secondary),
  ));
...
kazagumo.on("playerEmpty", async (player) => {
  ...
  if (old) {
    await old.delete().catch(async () => {
      try { await old.edit({ components: [] }); } catch {}
    });
